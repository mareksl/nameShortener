    ...
    var shorten = function(value, options, maxlen) {
      ...
      if (len <= maxlen) {
        ...
        for (var prop in options) {
          if (options.hasOwnPropertyCI(prop) && prop.length > options[prop].replacements[0].length) {
            var regex = new RegExp(prop, 'gi');
            var match;
            while ((match = regex.exec(value)) != null) {
              var result = [];
              result.push(prop);
              result.push(options[match]);
...
var loadRules = (function() {
  var load = function(createRules) {
    ...
    if (typeof localStorage.localRules !== 'undefined') {
      ...
      xobj.open('GET', 'js/rules_new.json', true); // Replace 'my_data' with the path to your file
